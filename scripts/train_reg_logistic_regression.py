...
# test datas
def cross_validation_step_logistic_regression(y, tx, k_indices, k, initial_w, lambda_, gamma, degree, max_iter):
    ...
    weights, loss_train = reg_logistic_regression(y_validation, tx_validation, lambda_, initial_w, max_iter, gamma)
...
def best_hyperparameters_logistic_regression(y, tx, degrees, lambdas, gammas, k_fold, initial_w, max_iter, seed=1):
        ...
        for gamma in gammas:
            ...
            for lambda_ in lambdas:
                ...
                for k in range(k_fold):
                    _, _, loss_test = cross_validation_step_logistic_regression(y, tx, k_indices, k, initial_w, gamma,
                                                                                degree, max_iter)
                    losses_test_tmp.append(loss_test)

                losses_test_gamma.append(np.mean(losses_test_tmp))

            best_lambda_index = np.argmin(losses_test_gamma)
            losses_test.append()
